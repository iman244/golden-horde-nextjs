  ...
  const createPeerConnection = useCallback(({username, target_user}:{username: string; target_user: string}) => {
    ...
    peerConnection.onicecandidate = (event) => {
      if (event.candidate) {
        ...
        const addressMatch = candidateStr.match(/ ([0-9]{1,3}(?:\.[0-9]{1,3}){3}) ([0-9]{1,5})/);
        ...
        console.log('[ICE] Candidate gathered:', { candidateType, protocol, address, port, raw: candidateStr });
    ...
    peerConnection.onicegatheringstatechange = () => {
      if (peerConnection.iceGatheringState === 'complete') {
        ...
        if (!foundRelay) {
          console.warn('[ICE] No relay (TURN) candidates were gathered. TURN server may be misconfigured or unreachable.');
        } else {
          console.log('[ICE] At least one relay (TURN) candidate was gathered.');
        }
        // Special log for STUN (srflx) candidates
        const foundSrflx = iceCandidates.some(c => / typ srflx /.test(c));
        if (!foundSrflx) {
          console.warn('[ICE] No srflx (STUN) candidates were gathered. STUN server may be misconfigured, unreachable, or your network does not require NAT traversal.');
        } else {
          console.log('[ICE] At least one srflx (STUN) candidate was gathered.');
        }
